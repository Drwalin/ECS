...
void PagedByteArray::init(uint32_t element_size, uint32_t page_elements)
{
	...
	assert(page_elements == (1 << page_elements_bit_offset));
...
void *PagedByteArray::access(uint32_t element_id)
{
	...
	if (pages_capacity <= page) {
		...
		pages = (uint8_t **)realloc(pages, sizeof(uint8_t *) * size);
		memset(pages + pages_capacity, 0,
			   (size - pages_capacity) * sizeof(uint8_t *));
		pages_capacity = size;
	} else if (pages[page] == nullptr) {
		if (pages_count <= page) {
			pages_count = page + 1;
		}
	...
	uint32_t id = element_id ^ first_element_in_page(page);
	return (void *)&(pages[page][id * element_size]);
}

uint32_t PagedByteArray::page_id_from_element_id(uint32_t element_id)
{
	return element_id >> page_elements_bit_offset;
}
uint32_t PagedByteArray::first_element_in_page(uint32_t page_id)
{
...
size_t PagedByteArray::get_memory_usage()
{
	return sizeof(uint8_t *) * pages_capacity +
		   (element_size * page_elements) * present_pages_count;
}
} // namespace ecs
